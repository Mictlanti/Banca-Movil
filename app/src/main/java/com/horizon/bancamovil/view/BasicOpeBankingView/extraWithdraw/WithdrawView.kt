...
import com.horizon.bancamovil.view.BasicOpeBankingView.depositView.depositSites
...
fun WithdrawViewRoute(navController: NavController, viewModel: BankingViewModel) {
    ...
    BasicOpeStyle(
        ...
        viewModel,
        bottomBar = {
            WithDrawBottomAppBar(state, selectedSite) {
...
fun WithdrawalAlert(
    ...
    fun condition(index: Int) {
        if (index == 0) {
            if (isWithdrawal) viewModel.events(BankingEvents.DepositInEvent(code - commission)) else viewModel.events(
